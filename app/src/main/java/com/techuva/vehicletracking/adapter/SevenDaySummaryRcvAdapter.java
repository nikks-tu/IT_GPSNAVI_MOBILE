...
public class SevenDaySummaryRcvAdapter extends RecyclerView.Adapter<VehicleSummaryRCVHolder>
{// Recyclerview will extend to
    // recyclerview adapter
    private ArrayList<SevenDaysDistanceModel> list;
    private Context context;
    private VehicleSummaryRCVHolder listHolder;
    int selectedPosition=0;
    int defaultMonth=0;

    public SevenDaySummaryRcvAdapter(Context context, ArrayList<SevenDaysDistanceModel> list, int position) {
    ...
    public void onBindViewHolder(VehicleSummaryRCVHolder holder, int position) {
        SevenDaysDistanceModel model = list.get(position);
        String dateToParse = model.getTRAVELLEDON();
