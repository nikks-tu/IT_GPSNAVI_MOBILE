...
import com.techuva.vehicletracking.holders.AccountsNameRCVHolder;
...
public class DeviceListRcvAdapter extends RecyclerView.Adapter<AvailableDeviceRCVHolder>
{
    private ArrayList<AvailableDevicesModel> arrayList;
    ...
    public DeviceListRcvAdapter(Context context, ArrayList<AvailableDevicesModel> arrayList, OnItemClicked listener) {
    ...
    public void onBindViewHolder(AvailableDeviceRCVHolder holder, int position) {
        AvailableDevicesModel model = arrayList.get(position);
        pos = position;

        holder.tv_tag_name.setText(model.getDISPLAYNAME());
        holder.tv_serial_num.setText(String.valueOf(model.getSERIALNO()));


        holder.ll_tag.setOnClickListener(v -> {
            MApplication.setString(context, Constants.INVENTORY_ID, String.valueOf(model.getINVENTORYID()));
