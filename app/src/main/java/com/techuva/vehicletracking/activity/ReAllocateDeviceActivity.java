...
public class ReAllocateDeviceActivity extends AppCompatActivity implements DeviceListRcvAdapter.OnItemClicked {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv_allocate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(INVENTORY_ID!=0){
                    serviceCallForReAllocateDevice();
                }
            }
        });
    ...
    private void init() {
        context = ReAllocateDeviceActivity.this;
        ...
        tv_title.setText(context.getResources().getString(R.string.device_re_allocation_));
                    ...
                    if(info.get("errorCode").getAsInt()==0)
                    {
                        ...
                        if(jsonArray.size()>0)
                        {
                            for (int i=0; i<jsonArray.size(); i++)
                            {
                                ...
                                if(!jObj.get("Condition").isJsonNull() && !jObj.get("Condition").getAsString().equals("")
                                && !jObj.get("Condition").getAsString().equals(""))
                                {
    ...
    private void serviceCallForReAllocateDevice() {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(Constants.BASE_URL_TOKEN)
                .addConverterFactory(ScalarsConverterFactory.create())
                .addConverterFactory(GsonConverterFactory.create())
        ...
        AllVehicleInfoDataInterface service = retrofit.create(AllVehicleInfoDataInterface .class);
        Call<JsonElement> call = service.reAllocateDevice(userId, authorityKey, new DeviceReAllocationPostParameters(VEHICLE_TAG_MAP_ID, INVENTORY_ID, REMARKS));
    ...
    public void showLoaderNew() {
        runOnUiThread(new ReAllocateDeviceActivity.Runloader(getResources().getString(R.string.loading)));
    }
    ...
    private void serviceCallforInfo() {
       ...
       int vehicleMasterId = Integer.parseInt(MApplication.getString(context, Constants.VehicleMasterID));
