...
public class AllocateDriverActivity extends AppCompatActivity implements DriverListRcvAdapter.OnItemClicked {
    ...
    private void init() {
        context = AllocateDriverActivity.this;
        ...
        VEHICLE_MASTER_ID = Integer.parseInt(MApplication.getString(context, Constants.VehicleMasterID));
        tv_title.setText(context.getResources().getString(R.string.driver_allocation));
                        ...
                        if(jsonArray.size()>0)
                        {
                            for (int i=0; i<jsonArray.size(); i++)
                            {
                                ...
                                if(!jObj.get("DRIVER_NAME").isJsonNull())
                                {
                                    ...
                                    tv_device_serial_num.setText(context.getResources().getString(R.string.driver_name_)+"--");
                                }
    ...
    private void serviceCallForAllocateDriver() {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl(Constants.BASE_URL_TOKEN)
                .addConverterFactory(ScalarsConverterFactory.create())
                .addConverterFactory(GsonConverterFactory.create())
        ...
        AllVehicleInfoDataInterface service = retrofit.create(AllVehicleInfoDataInterface .class);
        Call<JsonElement> call = service.allocateDriver(userId, authorityKey, new DriverAllocationPostParameters(VEHICLE_MASTER_ID, VEHICLE_DRIVER_ID, REMARKS));
    ...
    public void showLoaderNew() {
        runOnUiThread(new AllocateDriverActivity.Runloader(getResources().getString(R.string.loading)));
    }
